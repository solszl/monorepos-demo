...
fetchData(SERIES_ID).then(async (json) => {
  ...
  standard.on("slice_changed", () => {
    ...
    // markline.setData({ path: testData });
    standard.toolView.renderStaticData({
      type: "markline",
      path: testData,
    });
  });
...
document.addEventListener("wheel", async (e) => {
  ...
  // standard.imageView.showImage(image);
  standard.showImage(SERIES_ID, currentIndex);
});
...
const inputGap = document.querySelector("#inputGap");
inputGap.addEventListener("change", (e) => {
  standard.toolView.updateData({
    layerId: "staticLayer",
    toolId: "markline",
    props: {
      gap: +inputGap.value,
...
const inputOffset = document.querySelector("#inputOffset");
